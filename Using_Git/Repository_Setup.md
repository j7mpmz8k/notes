...
*This document instructs you on all steps needed to get Git set up on your computer and cloning a project.  If you have already cloned the homework's starter code, you may be better served by [Basic Git Workflow](./Basic_Git_Workflow.md).*
...
You can avoid the tedious entry of username and password by creating an SSH key and importing it into your GitLab profile.  

*   CS 1440 students will create an SSH key as part of the first assignment, [Shell Tutor](https://gitlab.cs.usu.edu/duckiecorp/shell-tutor)
*   If you need to (re)create an SSH key after you've done the Shell Tutor, the [SSH Key Setup Tool](https://gitlab.cs.usu.edu/duckiecorp/ssh-key-setup) will quickly get you running

If you encounter errors at this stage, follow the instructions in the Git [Troubleshooting Guide](./Troubleshooting.md)
...
3.  Clone the git repository containing the starter code from GitLab onto your computer using the `git clone` command.  The *local* repository's name on your own computer isn't terribly important as long as you can keep things straight.
    ...
    ```
    $ git clone git@gitlab.cs.usu.edu:duckiecorp/cs1440-falor-erik-assn0 cs1440-LAST-FIRST-assn0
    ```
...
6.  Return to your command shell and ask `git` about the status of your repository.
    ```
    $ git status
    ```
